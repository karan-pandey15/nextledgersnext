...
const CANADA_NAV_LINKS = [
  ...
  ABOUT_NAV_LINK,
  {
    ...
    href: "/canada#services",
    hasDropdown: true,
    dropdownItems: CANADA_SERVICE_LINKS,
  },
...
export default function CanadaNavbar({ isSidebarOpen = false, setIsSidebarOpen }) {
  ...
  return (
    <header className="w-full bg-white border-b border-gray-100 sticky top-0 z-40 overflow-visible">
      ...
      <style>{`
        ...
        .animate-canada-nav-dropdown {
          animation: canadaNavDropdownSlideIn 0.22s cubic-bezier(0.16, 1, 0.3, 1) forwards;
        }
        .canada-no-scrollbar::-webkit-scrollbar {
          display: none;
        }
        .canada-no-scrollbar {
          ...
          <nav className="hidden lg:flex items-center space-x-0.5 xl:space-x-1">
            {navLinks.map((link) => {
              ...
              const childActive = link.dropdownItems?.some((item) =>
                isNavLinkActive(pathname, item.href, CANADA_HOME)
              );
              ...
              if (link.hasDropdown) {
                return (
                  ...
                  >
                    ...
                    {isOpen && (
                      <div
                        className={`absolute left-0 mt-2 ${link.dropdownWidth || "w-[380px]"} origin-top-left rounded-[20px] bg-white border border-gray-100 p-4 shadow-[0_12px_45px_rgba(0,0,0,0.08)] z-50 animate-canada-nav-dropdown`}
                        onMouseEnter={() => handleMouseEnter(link.id)}
                        onMouseLeave={() => handleMouseLeave(link.id)}
                      >
                        <div className="flex flex-col gap-0.5">
                          {link.id === "services" && !isCanadaHome && (
                            <Link
                              href="/canada"
                          ...
                          {link.id === "services" && !isCanadaHome && (
                            ...
                            >
                              <img
                                ...
                                className="w-7 h-[18px] object-cover flex-shrink-0 rounded-xs"
                              />
                              <span className="text-xs font-black">Back To Canada</span>
                            </Link>
                          )}
